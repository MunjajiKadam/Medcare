...
export default function Navbar() {
  ...
  const getNavigation = () => {
    // Only return public links for non-logged-in users
    if (!user) return publicLinks;
    // Return empty array for logged-in users
    return [];
  };
      ...
      {mobileOpen && user && (
        <div className="md:hidden bg-background border-t border-gray-200 p-4 space-y-2">
          <Link
            ...
            onClick={() => setMobileOpen(false)}
            className="block px-4 py-2 bg-blue-600 text-white rounded text-center font-semibold hover:bg-blue-700"
          >
            📊 Dashboard
          </Link>
          <Link
