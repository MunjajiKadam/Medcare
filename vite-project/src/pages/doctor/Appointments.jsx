...
export default function Appointments() {
  ...
  if (loading) return (
    <>
      <Navbar />
      <div className="min-h-screen bg-background dark:bg-gray-900 flex items-center justify-center"><p>Loading appointments...</p></div>
      <Footer />
    </>
  );
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-background dark:bg-gray-900 p-6">
        ...
        <p className="text-gray-600 dark:text-gray-300 mb-6">View and manage patient appointments</p>
        ...
        {filteredAppointments.length === 0 ? (
          <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow dark:shadow-gray-900/50 text-center">
            <p className="text-gray-600 dark:text-gray-400">No {filter !== "all" ? filter : ""} appointments scheduled.</p>
          </div>
        ) : (
          <div className="grid gap-4">
            {filteredAppointments.map((apt) => (
              <div key={apt.id} className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md dark:shadow-gray-900/50 border-l-4 border-secondary hover:shadow-lg dark:hover:shadow-gray-900/70 transition">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex-1">
                    <h3 className="text-lg font-bold text-dark">{apt.patient_name || "Patient"}</h3>
                    <p className="text-sm text-gray-600 dark:text-gray-300">📅 {new Date(apt.appointment_date).toLocaleDateString()} at {formatTime12Hour(apt.appointment_time)}</p>
                    <p className="text-sm text-gray-600 dark:text-gray-300 mt-2">📝 {apt.reason_for_visit || "General checkup"}</p>
                    {apt.symptoms && <p className="text-sm text-gray-600 dark:text-gray-300">🩹 {apt.symptoms}</p>}
                  </div>
